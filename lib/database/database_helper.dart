...
class DatabaseHelper {
  ...
  Future<int> cadastrarUsuario(
    ...
    try {
      ...
      if (e.isUniqueConstraintError()) {
        throw Exception('Email já cadastrado. Use outro email.');
      }
      rethrow;
    }
  }
  ...
  Future<List<Map<String, dynamic>>> getTreinosPorUsuario(
    int usuarioId,
    String objetivo,
  ) async {
    final db = await instance.database;
    ...
    return await db.query(
      'treinos',
      where: 'usuario_id = ? AND objetivo = ?',
      whereArgs: [usuarioId, objetivo],
      orderBy: 'id',
    );
  }

  Future<int> atualizarTreino(
    int id, {
    String? dia,
    String? exercicio,
    String? series,
    ...
    if (dia != null) values['dia'] = dia;
    if (exercicio != null) values['exercicio'] = exercicio;
    if (series != null) values['series'] = series;

    if (values.isEmpty) return 0;

    return await db.update('treinos', values, where: 'id = ?', whereArgs: [id]);
  }

  Future<int> excluirTreino(int id) async {
    final db = await instance.database;
